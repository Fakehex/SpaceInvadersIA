...
def policy_backward(eph, epdlogp):
  ...
  dh[eph <= 0] = 0 # backpro prelu, fonction d'activation
  dW1 = np.dot(dh.T, epx)
...
while True:
  ...
  action = 4 if np.random.uniform() < aprob else 5 # roll the dice!

  # record various intermediates (needed later for backprop)
  xs.append(x) # observation
  hs.append(h) # hidden state
  y = 1 if action == 4 else 0 # a "fake label" 
  dlogps.append(y - aprob) # grad that encourages the action that was taken to be taken (see http://cs231n.github.io/neural-networks-2/#losses if confused) stock des loss
